...
function mostrarPosts() {
  const q2 = Rl(_h(db, "post"), xl("dateCreate", "desc"));
  return q2;
}
function eliminarPost(id2) {
  yf(gh(db, "post", id2));
}
function editarPost(id2, texto) {
  const miRef = gh(db, "post", id2);
  return gf(miRef, {
    text: texto
  });
}
const home = () => {
  ...
  email.placeholder = "Ingresa tu correo aqu\xED";
  password.placeholder = "Y tu contrase\xF1a aqu\xED";
  btnSignIn.textContent = "Inicia sesi\xF3n";
  btnGoogle.textContent = "Inicia sesi\xF3n con google";
  ...
  article.append(h1Home, formHome, email, password, btnSignIn, btnGoogle, btnRegister);
  return article;
};
const register = () => {
...
const wall = () => {
  ...
  crearPost.placeholder = "\xBFQu\xE9 quieres publicar?";
  ...
  If(mostrarPosts(), (querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      p2.className = "p";
      p2.textContent = doc.data().text;
      p2.setAttribute("data-userid", doc.data().uid);
    ...
    allbuttons.forEach((btn) => {
      ...
      btn.addEventListener("click", (e) => {
        ...
        console.log(id2, "******");
        const opcion = confirm("\xBFEst\xE1s seguro que quieres eliminar este post?");
        console.log(opcion);
        if (opcion === true) {
          eliminarPost(id2);
        }
  ...
  articleFlexWall.append(h1, h2, crearPost, btnEnviar, articleComments);
